...
static int read_cmd(char *buffer)
{
    ...
    while (1) { // if enter is pressed issue command
        ch[0] = keyboard_readu();
...
int main(void)
{
    char buffer[80] = { 0 }; // buffer that will hold data of console
    int buffer_size = 0; // command length
    int cmd = 0; // index of commands we support
